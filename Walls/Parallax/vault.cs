        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            if (ShouldDrawRailing(i, j))
            {
                ...
                if (left || right)
                {
                    ...
                    Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Walls/Parallax/railing").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, (j - 1) * 16 - (int)Main.screenPosition.Y) + zero, frame, Lighting.GetColor(i, j), 0f, Vector2.Zero, 1f, 0, 0f);
                }
            }
        }
