...
namespace Remnants.Walls.Parallax
{
    public class GoldenCity : ModWall
    {
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (!tile.HasTile || !Main.tileBlockLight[tile.TileType] || tile.Slope != SlopeType.Solid || tile.IsHalfBlock)
            {
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>("Remnants/Walls/Parallax/goldencityglow").Value, new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y) + zero, frame, RemTile.GoldenCityLightColour(i, j, red: true), 0f, Vector2.Zero, 1f, 0, 0f);
            }
    ...
    public class goldenmonitor : ModWall
    {
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            Texture2D texture = ModContent.Request<Texture2D>("Remnants/Walls/Backgrounds/goldenmonitor").Value;
            Vector2 zero = Main.drawToScreen ? Vector2.Zero : new(Main.offScreenRange);
    ...
    public class goldenconsole : ModWall
    {
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            Texture2D texture = ModContent.Request<Texture2D>("Remnants/Walls/Backgrounds/goldenconsole").Value;
            Vector2 zero = Main.drawToScreen ? Vector2.Zero : new(Main.offScreenRange);
            Rectangle frame = new(16, 16, 16, 16);
    ...
    public class goldenserver : ModWall
    {
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch)
        {
            Texture2D texture = ModContent.Request<Texture2D>("Remnants/Walls/Backgrounds/goldenserver").Value;
            Vector2 zero = Main.drawToScreen ? Vector2.Zero : new(Main.offScreenRange);
