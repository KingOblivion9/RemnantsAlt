...
namespace Remnants.World
{
    public class JungleTemple : GenPass
    {
        ...
        protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            Generator.GenerateStructure("Structures/special/temple/endroom", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
            ...
            for (cellY = 0; cellY < roomsVertical; cellY++)
            {
                for (cellX = roomsLeft; cellX <= roomsRight; cellX++)
                {
                    if (!FindMarker(cellX, cellY) && !FindMarker(cellX, cellY, 1))
                    {
                        ...
                        if (roomID == 1)
                        {
                            Generator.GenerateStructure("Structures/special/temple/a1", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        }
                        if (roomID == 2)
                        {
                            Generator.GenerateStructure("Structures/special/temple/a2", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        }
                        if (roomID == 3)
                        {
                            Generator.GenerateStructure("Structures/special/temple/a3", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        }
                        if (roomID == 4)
                        {
                            Generator.GenerateStructure("Structures/special/temple/b1", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        }
                        if (roomID == 5)
                        {
                            Generator.GenerateStructure("Structures/special/temple/b2", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        }
                        if (roomID == 6)
                        {
                            Generator.GenerateStructure("Structures/special/temple/b3", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        }
                        ...
                        //    posX = X + cellX * roomWidth - roomWidth / 2 + 1;
                        //    posY = Y + cellY * roomHeight - roomHeight / 2 + 1;
                        //    Generator.GenerateStructure("Structures/special/temple/4way", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        //}
                        //else
                        //{
                ...
                for (cellX = roomsLeft; cellX <= roomsRight; cellX++)
                {
                    if (!FindMarker(cellX, cellY, 1))
                    {
                        if (FindMarker(cellX - 1, cellY, 1))
                        {
                            ...
                            posY = Y + cellY * roomHeight - roomHeight / 2 + 1;
                            Generator.GenerateStructure("Structures/special/temple/corner-left", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                        ...
                        if (FindMarker(cellX + 1, cellY, 1))
                        {
                            posX = X + (cellX + 1) * roomWidth - roomWidth / 2 + 1;
                            posY = Y + cellY * roomHeight - roomHeight / 2 + 1;
                            Generator.GenerateStructure("Structures/special/temple/corner-right", new Point16(posX, posY), ModContent.GetInstance<Remnants>());
                            if (cellY >= roomsVertical - 1)
                            {
                                DungeonRoom(1, 1, roomWidth, 21, cellX + 1, cellY + 1, tile: TileID.LihzahrdBrick);
                        ...
                        if (cellX == 0 && FindMarker(cellX, cellY - 1, 1))
                        {
                            ...
                            Generator.GenerateStructure("Structures/special/temple/top", new Point16(posX, posY + 1), ModContent.GetInstance<Remnants>());
            ...
            if (!devMode)
            {
                ...
                while (objects > 0)
                {
                    ...
                    bool valid = true;
                    if (WGTools.Tile(x, y).LiquidAmount > 0 || WGTools.Tile(x, y).WallType != ModContent.WallType<Walls.Parallax.temple>() && WGTools.Tile(x, y).WallType != WallID.LihzahrdBrickUnsafe)
                    {
                        valid = false;
                    }
